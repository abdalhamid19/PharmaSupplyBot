...
from src.core.ordering.prevented_items import PreventedItem, load_prevented_items
...
class StreamlitOrderTests(unittest.TestCase):
    ...
    def test_order_run_summary_path_uses_match_only_summary(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("src.ui.order.streamlit_order_form.ARTIFACTS_DIR", artifacts):
    ...
    def test_prevented_excel_options_reads_prevented_items_directory(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "src.ui.views.streamlit_prevented_items.PREVENTED_ITEMS_DIR", prevented_dir
            ):
    ...
    def test_render_order_tab_rejects_prevented_file_as_order_excel(self) -> None:
        ...
        with (
            patch(
                "src.ui.order.streamlit_order.render_running_order_controls",
                return_value=False,
            ),
            patch(
                "src.ui.order.streamlit_order.order_form_values",
                return_value=(True, form_values),
            ),
            patch("src.ui.order.streamlit_order.st.subheader"),
            patch("src.ui.order.streamlit_order.st.error") as error,
            patch("src.ui.order.streamlit_order.run_order_submission") as run_submission,
        ):
