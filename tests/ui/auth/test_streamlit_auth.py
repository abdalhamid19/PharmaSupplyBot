...
from src.ui.auth.streamlit_auth import interactive_auth_available


class StreamlitAuthTests(unittest.TestCase):
    def test_interactive_auth_available_on_windows(self) -> None:
        with patch("src.ui.auth.streamlit_auth.sys.platform", "win32"):
    ...
    def test_interactive_auth_unavailable_on_linux_without_display(self) -> None:
        with patch("src.ui.auth.streamlit_auth.sys.platform", "linux"):
            with patch.dict(os.environ, {}, clear=True):
