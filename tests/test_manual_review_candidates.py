...
from src.core.manual_review_candidate_store import (
    append_review_candidates,
    load_review_candidates,
)
from src.core.manual_review_candidates import (
...
class ManualReviewCandidatesTests(TestCase):
    ...
    def test_extract_options_respects_limit(self) -> None:
        diag = CandidateMatchDiagnostic(
            query="item", row_index=0, score=10.0, sort_key=(10.0,), accepted=False,
            accepted_reason="", rejection_reason="", breakdown=None, candidate={},
        )
        decision = MatchDecision(best_match=None, diagnostics=[diag] * 10, final_reason="")
    ...
    def test_store_appends_and_loads_candidates(self) -> None:
        options = [
            ReviewCandidateOption(
                store_product_id="s1", name_en="Med", name_ar="ميد", supplier="Supp",
                available_quantity=10, price=20.0, score=5.0, rejection_reason="bad", orderable=True
            )
        ]
