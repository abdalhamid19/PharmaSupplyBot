...
| **سبب ثانوي مُكتشَف (H1b)** | مُستخرج الشركة المصنّعة `extract_manufacturer_from_name()` معطوب — يعتبر آخر كلمة في اسم الدواء "شركة مصنّعة" (مثل `EXTRA` من `PANADOL EXTRA`)، مما يسبب تضارباً وهمياً مع `companyName` الحقيقي ويمنع الحفظ حتى بعد إصلاح الـ tuple. |
| **الحل المُطبَّق (S2)** | 1) فك الـ tuple واستخدام القيمة المنطقية فقط. 2) ربط فحص الشركة المصنّعة التقريبي بفلاغ `enable_manufacturer_check` (افتراضياً معطّل) بدل تشغيله دائماً. 3) تمرير سبب الرفض الحقيقي من `diagnostics`. 4) تسجيل سبب التخطي في الـ log بدل الفشل الصامت. |
| **الاختبارات** | 49 اختبار جديد (Reproduction + Hypotheses + Solutions + PostFix) — كلها ناجحة. الحزمة الكاملة: **682 ناجح** بنفس عدد الإخفاقات السابقة للتغيير (8 إخفاقات موجودة مسبقاً في baseline). |
