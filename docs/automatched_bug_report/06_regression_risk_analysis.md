...
### 3.1 إصلاح بنيوي لـ `extract_manufacturer_from_name` (أولوية عالية) — ✅ **مُطبَّق**

المشكلة البنيوية: الخوارزمية القديمة "آخر token غير عام = الشركة المصنّعة" مبنية على افتراض خاطئ عن تسمية الأدوية.

خيارات التصميم التي دُرست:
...
**نُفِّذ الخيار 2** (مع تعرُّف على قائمة `KNOWN_MANUFACTURERS` المُنسَّقة في جانب الصنف).
القياس على 1232 اسم صنف حقيقي أثبت أن الخوارزمية القديمة اختلقت شركة في **99% من الحالات**.
التفاصيل الكاملة والقياسات والاختبارات الـ 17: **[08_manufacturer_identity_fix.md](08_manufacturer_identity_fix.md)**.
