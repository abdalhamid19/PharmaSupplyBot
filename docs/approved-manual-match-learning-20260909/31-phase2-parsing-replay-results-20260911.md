# نتيجة Phase 2: parsing وترتيب المرشحين

## التغيير الأول: parsing
...
## التغيير الثاني: ترتيب compatibility

بعد إثبات أن `VOLTAREN 3AMP` متوافق من ناحية form/pack، أظهر replay أن المرشح
الصحيح كان يتأخر بسبب تقديم `ranking_tier` على compatibility. تم تعديل الترتيب
ليعطي الأولوية للتوافق الصريح، ثم tier، ثم score وmargin وrow key. هذا لا يغيّر
`ranking_tier` ولا evidence kind، ولا يفتح automatic matching.

مقارنة `20260911_1559` قبل ترتيب compatibility مع `20260911_1609` بعده:

| الهدف | recall@1 قبل | recall@1 بعد | recall@3 بعد | matched قبل/بعد |
|---|---:|---:|---:|---:|
| البركة شركات | 0% | 50% | 100% | 154 / 154 |
| القيصر شركات | 100% | 100% | 100% | 109 / 109 |

في البركة أصبح `فولتارين 3مبول س جديد` أول مرشح لـ`VOLTAREN 3AMP`. بقيت حالة
XITHRONE في المرتبة الثانية لأن الاسم العربي لا يثبت `500MG`، وهو تحفظ مقصود.
عدد المرشحين المحفوظين وعدد manual review لم يتغيرا.
الخطوة التالية هي قياس `C_display` وround-trip في UI، ثم توسيع labels البشرية
قبل أي تعديل thresholds أو توسيع retrieval.
