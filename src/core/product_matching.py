...
def _safe_omitted_liquid_concentration(tokens: set[str], requested, offered) -> bool:
    """Safe when the query has the MG strength and the candidate just adds /Y ML.

    Example: query "AUGMENTIN 457 SUSP" vs candidate "AUGMENTIN 457 MG / 5 ML SUSP"
    => extra token {5} is the per-ML volume denominator, safe to ignore.
    """
...
def _safe_omitted_pack_count(tokens: set[str], requested, offered) -> bool:
    """Safe when the query has matching dosage and the only extra is pack count.

    Example: query "BRUFEN 400 TAB" vs candidate "BRUFEN 400 MG 30 TABS."
    => extra token {30} is the pack count, safe to ignore.
    """
    if not _dosage_or_shared_strength(requested, offered):
        return False
    if not offered.qty:
...
def _dosage_or_shared_strength(requested, offered) -> bool:
    """True when both sides share a strength number.

    The parser may classify a bare number like '400' in 'BRUFEN 400 TAB' as
    qty rather than dosage.  So we also check whether any query number appears
    in the candidate's dosage_nums, which proves they share a strength.
    """
...
def _safe_omitted_pen_insulin_details(tokens: set[str], requested, offered) -> bool:
    """Safe when candidate adds IU/pen count details the query omitted.

    Only when both share a PEN/CARTRIDGE form.
    """
